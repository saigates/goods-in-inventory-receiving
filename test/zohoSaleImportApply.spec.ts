...
// NOTE: seeds a real RECEIVED received_devices row for every generated
// IMEI -- the INNER JOIN CONTRACT means an unmatched serial produces NO
// outcome at all (not even skipped_available), so a row with no
// corresponding device would silently vanish from every count rather
// than showing up as skippedAvailableCount.
async function makeSkippedAvailableRows(n: number): Promise<ZohoCsvRow[]> {
  const rows: ZohoCsvRow[] = []
  for (let i = 0; i < n; i++) {
    const device = await seedDevice('RECEIVED')
    rows.push(makeRow(device.imei, {
...
describe('applyZohoSaleImport — LOW-YIELD acknowledgment gate (2026-09-10, replaces retracted assertOutwardsShape)', () => {
  it('(i) zero_sales: a >=50-row file with zero matched_sale outcomes triggers lowYield and a real run writes NOTHING without the flag', async () => {
    ...
    const rows = await makeSkippedAvailableRows(50)
  ...
  it('(ii) high_skipped_available_ratio: a >=50-row file with skipped_available >= 25% (but >0 sales) triggers lowYield and writes nothing without the flag', async () => {
    ...
    const skippedRows = await makeSkippedAvailableRows(40)
    const saleDevices = await Promise.all(Array.from({ length: 10 }, () => seedDevice('RECEIVED')))
    const saleRows = saleDevices.map((device, i) =>
      makeRow(device.imei, { out_entity_number: `INV-RATIO-${i}`, sold_price: '99.00' })
    )
    ...
    for (const device of saleDevices) {
      const dev = await deviceRow(device.id)
      expect(dev.status).toBe('RECEIVED')
    }
  })
  ...
  it('(iv) a file under LOW_YIELD_MIN_ROWS (49 rows, all skipped_available) is NEVER judged -- lowYield stays null even though it would otherwise trip zero_sales', async () => {
    const rows = await makeSkippedAvailableRows(49)
    const csv = csvOf(rows)
