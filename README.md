...
- **CSV export hardening still deliberately NOT done**: values are written byte-faithfully, so a device field beginning `=`, `+`, `-` or `@` is still a live formula when the file is opened in Excel (CSV-injection). Not mitigated because prefixing a `'` would corrupt the audit value; the correct fix is an `.xlsx` export that carries types explicitly. Treat exported CSVs as untrusted input if any field is ever operator-free-text.
