...
| `POST` | `/api/opr/shipments/:id/lines` | 🔒 | Add a device: snapshots IMEI/SKU/attributes/`buy_price` **frozen at add time** (later device edits never leak into the declared line). Devices without `buy_price` rejected (422); one line per device per shipment (409) |
| `DELETE` | `/api/opr/shipments/:id/lines/:lineId` | 🔒 | Remove a line (DRAFT only) |
...
**Automated suite**: [Vitest](https://vitest.dev/) via [`@cloudflare/vitest-pool-workers`](https://developers.cloudflare.com/workers/testing/vitest-integration/), which runs tests inside the real `workerd`/Miniflare runtime against a real D1 binding (all 10 migrations in `migrations/` applied before each test file — see `vitest.config.ts` and `test/apply-migrations.ts`), not mocks.
...
Current coverage (119 tests across 5 suites):
...
- Export/return/sale lifecycle transitions (`READY_FOR_EXPORT → IN_EXPORT_CONSIGNMENT → EXPORTED_UNDER_OPR → RETURNED_UNDER_OPR`, `→ SOLD`) — the statuses exist in the enum but no transitions are wired to them yet; they belong to OPR 2 finalisation.
- **OPR 2–4** (OPR 1 foundation entities are done, API-level): consignment-builder UI, the green/amber/red validation engine, commercial-invoice + scan-out list + pre-alert draft generation, finalisation, C&E1154 generator, re-import/discharge tracking, Gmail send. The seeded Saigates authorisation and the `shipments`/`shipment_lines` entities are the substrate these build on.
- OPR UI — the `/api/opr/*` endpoints have no frontend yet; the SPA does not surface authorisations or shipments.
